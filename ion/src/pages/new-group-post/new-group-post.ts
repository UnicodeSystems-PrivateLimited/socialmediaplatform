...
import { CustomsharePage } from '../customshare/customshare';
import { Camera, FileChooser, File, FilePath,MediaCapture,MediaFile } from 'ionic-native';
declare function unescape(s: string): string;
/*
  Generated class for the NewGroupPost page.

  See http://ionicframework.com/docs/v2/components/#navigation for more info on
  Ionic pages and navigation.
*/
@Component({
  selector: 'page-new-group-post',
  templateUrl: 'new-group-post.html',
  providers: [UserService, CommonService, DataService, PageService, MyWallService]

})
export class NewGroupPostPage {
  ...
  /**Posts */
  public _dataUploadPhotosFiles = '/api/group_post/postPhotosTypeFiles/';
  public _dataUploadDocumentsFiles = '/api/group_post/postDocumentsTypeFiles/';
  public _dataUploadAudiosFiles = '/api/group_post/postAudiosTypeFiles/';
  public _dataUploadVideosFiles = '/api/group_post/postVideosTypeFiles/';
  public _addPostUrl = '/api/group_post/postAllTypeData/';
  public _dataUploadEmbedLink = '/api/group_post/postVideoEmbedLink/';
  public _dataUploadAudioEmbedLink = '/api/group_post/postAudioEmbedLink/';
  public _dataUploadPhotoEmbedLink = '/api/group_post/postPhotoEmbedLink/';
  public notiMsg: string = 'Enter your message';
  public groupPostData: Post = new Post();
  ...
  constructor(public navCtrl: NavController,
    ...
    this.isMember = navParams.data.isMember;
  ...
  cancel() {
    if (this.wallType == 'groupWall') {
      let data = { wallId: this.groupId };
  ...
  onPostAllTypeData() {
    if (this.isMember) {
      this.groupPostData.privacy = this.selectedPostPrivacyType;
      this.groupPostData.types = this.types;
      this.groupPostData.catagory = this.catagory;
      this.groupPostData.created_on = new Date();
      this.groupPostData.custom = this.selectedPostPrivacyType == 5 ? this.taggedUsersIds : [];
      if (typeof (this.photosToUpload) != 'undefined' && this.photosToUpload.length > 0) {
        ...
        this.loader = true;
        this.groupPostData.photo = this.photosToUpload;
        this.groupPostData.post_type = 3;
        this.groupPostData.name = this.message.name.name ? this.message.name.name.replace(/(^[ \t]*\n)/gm, "") : '';
        this.dataService.wallPostFormData(this._dataUploadPhotosFiles + this.groupId, this.groupPostData)
          .subscribe((result) => {
            if (result['status'] == 2) {
              this.loader = false;
              this.photosToUpload = [];
              this.groupPostData = new Post();
              this.message.name.name = '';
              this.catagory = 1;
        ...
        { this.message.name.name = null; }
        this.loader = true;
        this.groupPostData.post_type = 4;
        this.groupPostData.name = this.message.name.name ? this.message.name.name.replace(/(^[ \t]*\n)/gm, "") : '';
        this.groupPostData.video = this.videosToUpload;
        this.dataService.wallPostFormData(this._dataUploadVideosFiles + this.groupId, this.groupPostData).subscribe((result) => {
          if (result['status'] == 2) {
            this.taggedUsersIds = [];
            this.selectedPostPrivacyType = 1;
            this.groupPostData = new Post();
        ...
        this.loader = true;
        this.groupPostData.name = this.message.name.name ? this.message.name.name.replace(/(^[ \t]*\n)/gm, "") : '';
        this.groupPostData.audio = this.audiosToUpload;
        this.groupPostData.post_type = 6;
        this.dataService.wallPostFormData(this._dataUploadAudiosFiles + this.groupId, this.groupPostData).subscribe((result) => {
          if (result['status'] == 2) {
            this.taggedUsersIds = [];
            this.selectedPostPrivacyType = 1;
            this.groupPostData = new Post();
        ...
        { this.message.name.name = null; }
        this.loader = true;
        this.groupPostData.post_type = 7;
        this.groupPostData.name = this.message.name.name ? this.message.name.name.replace(/(^[ \t]*\n)/gm, "") : '';
        this.groupPostData.document = this.documentsToUpload;
        this.dataService.wallPostFormData(this._dataUploadDocumentsFiles + this.groupId, this.groupPostData).subscribe((result) => {
          if (result['status'] == 2) {
            this.taggedUsersIds = [];
            this.selectedPostPrivacyType = 1;
            this.groupPostData = new Post();
        ...
        { this.message.name.name = null; }
        this.loader = true;
        this.groupPostData.post_type = 5;
        this.groupPostData.name = this.message.name.name ? this.message.name.name.replace(/(^[ \t]*\n)/gm, "") : '';
        this.groupPostData.link = this.message.embedLink.video;
        this.dataService.postData(this._dataUploadEmbedLink + this.groupId, this.groupPostData).subscribe(posts => {
          if (posts['status'] == 2) {
            this.taggedUsersIds = [];
            this.selectedPostPrivacyType = 1;
            this.groupPostData = new Post();
        ...
        this.groupPostData.post_type = 5;
        this.groupPostData.name = this.message.name.name ? this.message.name.name.replace(/(^[ \t]*\n)/gm, "") : '';
        this.groupPostData.link = this.message.embedLink.audio;
        this.dataService.postData(this._dataUploadAudioEmbedLink + this.groupId, this.groupPostData).subscribe(posts => {
          if (posts['status'] == 2) {
            this.taggedUsersIds = [];
            this.selectedPostPrivacyType = 1;
            this.groupPostData = new Post();
        ...
        { this.message.name.name = null; }
        this.loader = true;
        this.groupPostData.link = this.message.embedLink.photo;
        this.groupPostData.name = this.message.name.name ? this.message.name.name.replace(/(^[ \t]*\n)/gm, "") : '';
        this.groupPostData.post_type = 5;
        this.dataService.postData(this._dataUploadPhotoEmbedLink + this.groupId, this.groupPostData)
          .subscribe((posts) => {
            if (posts['status'] == 2) {
              this.taggedUsersIds = [];
              this.selectedPostPrivacyType = 1;
        ...
        if ((this.postMsgLinkQuesType !== 0) && (this.postMsgLinkQuesType === 1) && (this.message.message.message !== '')) {
          this.groupPostData.message = this.message.message.message ? this.message.message.message.replace(/(^[ \t]*\n)/gm, "") : '';
          this.groupPostData.post_type = 1;
          this.loader = true;
          this.dataService.postData(this._addPostUrl + this.groupId, this.groupPostData)
            .subscribe((posts) => {
              if (posts['status'] == 2) {
                this.message.message.message = '';
                this.selectedPostPrivacyType = 1;
          ...
          this.groupPostData.question = this.message.question.question ? this.message.question.question.replace(/(^[ \t]*\n)/gm, "") : '';
          this.groupPostData.post_type = 2;
          this.loader = true;
          this.dataService.postData(this._addPostUrl + this.groupId, this.groupPostData)
            .subscribe((posts) => {
              if (posts['status'] == 2) {
          ...
          if (this.validateLinkURL(this.message.link.description)) {
            this.groupPostData.link = this.message.link.description;
            this.groupPostData.linkTitle = this.message.link.title ? this.message.link.title.replace(/(^[ \t]*\n)/gm, "") : '';
            this.groupPostData.post_type = 5;
            this.loader = true;
            this.dataService.postData(this._addPostUrl + this.groupId, this.groupPostData)
              .subscribe((posts) => {
      ...
      this.commonService.showToast("Please Join Group For Posting!");
    }
  }
  ...
  selectVisibility($event) {
    this.groupPostData.privacy = $event;
  }

  customVisiblity(visibility) {
    this.groupPostData.privacy = visibility;
    let modal = this.modalCtrl.create(CustomsharePage);
    modal.onDidDismiss(data => {
      if (data && data.customPostId.length > 0 && data.custom.length > 0) {
        this.message.custom = data.custom;
        this.taggedUsersIds = data.customPostId;
      }else{
        this.selectedPostPrivacyType = 1;
        this.groupPostData.privacy = 1;
      }
    });
    modal.present();
  }
