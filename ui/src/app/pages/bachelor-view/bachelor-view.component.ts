...
@Component({
    selector: 'bachelor-view',
    template: require('./bachelor-view.html'),
    directives: [RouterOutlet,reportInfoComponent, LikeMemberListComponent, EmojiIcons, BaPostExternalShareModel, RouterLink, ROUTER_DIRECTIVES, TOOLTIP_DIRECTIVES, EmbedDirective, ProfileAssetDetailsComponent, BaPostShareModel, BaPostCustomPrivacyModel, BaPostEditModel, AddPostToJournalModal, Angular2AutoScroll],
    host: { 'class': 'ng-animate page1Container' },
    pipes: [documentNameFilterPipe, FollowUnfollowStatus, DateTime, TimeAgoPipe, LikeFilterPipe, DateFormatPipe, YoutubeSafeUrlPipe, CalendarPipe, TextFormattingPipe, nameFilterPipe]
})

export class BachelorView {
    ...
    public _postSearchUrl = '/api/degree_post/getAllDegreePostsByTitle/';
    ...
    private _saveLastVisitUrl = '/api/user/updateDegreeLastVisit';
    public setPostType = '';
    ...
    public degreeFriendCount: number = 0;
    public futureMemberCount: number = 0;
    public degreeFriend: Array<any> = [];
    ...
    public selectedDegreeStatus: number = 1;
    ...
    ngAfterViewInit() {
        setTimeout(function () {
            jQuery('input[name="search-date-from-1"]').datepicker({ autoclose: true, format: 'dd-mm-yyyy' });
            jQuery('input[name="search-date-to-1"]').datepicker({ autoclose: true, format: 'dd-mm-yyyy' });
        }, 5000);
    }
    ...
    resetSearch() {
        ...
        this.selectedDegreeStatus = 1;
        jQuery('input[name="search-date-from-1"]').val('');
        jQuery('input[name="search-date-to-1"]').val('');
    ...
    deletePostById() {
        this.dataService.getData(this.deletePostUrl + '/' + this.modal_delete_post_id + '/' + this.degreeId).subscribe(res => {
            if (res['status'] == 2) {
    ...
    getBachelorWall() {
        this.dataService.getData(this._dataUrlDegree + this.degreeId).subscribe(sub => {
            ...
            this.degreeFriend = sub.friend_members;
            this.degreeFriendCount = this.degreeFriend.length;
    ...
    getPosts(postType: number) {
        ...
        if (!this.searching) {
            ...
            this.searching = false;
            this.dataService.getData(this._getDataByPostTypeUrl + this.degreeId + '/' + this.postType + '/' + this.counterList).subscribe(res => {
                if (res.status == 2) {
                    if (res.data) {
                        this.post_data = res.data;
                    }
        ...
        if (startDate) {
            ...
            if (endDate) {
                ...
                if (moment(this.searchDateTo).isSameOrAfter(this.searchDateFrom)) {
                    ...
                    this.dataService.getData(this._getSearchUrl + this.degreeId + '/' + this.counterList + '/' + this.postType + '/' + this.searchDateFrom + '/' + this.searchDateTo + '/' + this.category + '/' + this.who_posted + '/' + this.selectDegreeStatus).subscribe(post => {
                        if (post.status == 2) {
                            if (post.data) {
    ...
    //post all type degree data
    onPostAllTypeDegreeData() {
        if (this.page.is_member) {
            ...
            if (typeof (this.photosToUpload) != 'undefined' && this.photosToUpload.length > 0) {
                ...
                if (this.error.photo == '') {
                    ...
                    this.dataService.postFormData(this._dataUploadPhotosFiles + this.degreeId, this.degreePostData).subscribe((result) => {
                        if (result['status'] == 2) {
                ...
                if (this.error.audio == '' && this.error.audioSize == '') {
                    ...
                    this.dataService.postFormData(this._dataUploadAudiosFiles + this.degreeId, this.degreePostData).subscribe((result) => {
                        if (result['status'] == 2) {
                            if (typeof (this.imageFile) != "undefined")
                                this.imageFile.target.value = "";
                            this.audiosToUpload = [];
                ...
                if (this.error.video == '' && this.error.videoSize == '') {
                    ...
                    this.dataService.postFormData(this._dataUploadVideosFiles + this.degreeId, this.degreePostData).subscribe((result) => {
                        if (result['status'] == 2) {
                            if (typeof (this.imageFile) != "undefined")
                                this.imageFile.target.value = "";
                            this.videosToUpload = [];
                ...
                if (this.error.document == '' && this.error.documentSize == '') {
                    ...
                    this.dataService.postFormData(this._dataUploadDocumentsFiles + this.degreeId, this.degreePostData).subscribe((result) => {
                        if (result['status'] == 2) {
                            if (typeof (this.imageFile) != "undefined")
                                this.imageFile.target.value = "";
                            this.documentsToUpload = [];
                ...
                this.dataService.postData(this._dataUploadPhotoEmbedLink + this.degreeId, this.degreePostData).subscribe(posts => {
                    if (posts['status'] == 2) {
                        this.message.embedLink.photo = '';
                        this.message.name.name = '';
                        this.selectedPostPrivacyType = 1;
                ...
                if ((this.postMsgLinkQuesType !== 0) && (this.postMsgLinkQuesType === 1) && (this.message.message.message !== '')) {
                    ...
                    this.dataService.postData(this._addPostUrl + this.degreeId, this.degreePostData).subscribe(posts => {
                        if (posts['status'] == 2) {
                            ...
                            this.degree = posts.data_message;
                            this.post_again = posts.post_again;
            ...
            var n = noty({ text: '<div class="alert bg-theme-dark"><p>Please Join Degree For Posting</p></div>', layout: 'center', theme: 'made', maxVisible: 10, animation: { open: 'animated fadeInLeft', close: 'animated fadeOutDown' }, timeout: 4000, });
        }
    }
    ...
    visibility(visible) {
        ...
        if (visible == 1) {
            ...
            if (visible == 2) {
                ...
                if (visible == 3) {
                    this.visibilityStatus = 'Friends Only'
                }
                else
                    if (visible == 4) {
                        this.visibilityStatus = 'Followers'
                    }
                    else
                        if (visible == 6) {
                            this.visibilityStatus = 'Friends And Followers'
                        }
                        else
                            if (visible == 5) {
                                this.visibilityStatus = 'Custom';
                ...
                if (this.scrollController) {
                    if (this.postType != null) {
                        ...
                        if (this.counterList <= (page + 1)) {
                            this.counterList++;
                            this.contentLoding = true;
                            this.dataService.getData(this._getDataByPostTypeUrl + this.degreeId + '/' + this.postType + '/' + this.counterList).subscribe(post => {
                                if (post.status == 2) {
                                    if (post.data) {
    ...
    public saveLastVisitTime(): void {
        this.dataService.getData(this._saveLastVisitUrl + '/' + this.degreeId).subscribe((res) => {
        });
    }
    ...
    public getGroups(): void {
        this.dataService.getData(this._getGroups + 3 + '/' + this.degreeId + '/' + 0).subscribe(res => {
            if (res.status == 2) {
                if (res.data) {
    ...
    public getGroupMember(id: number) {
        this.router.navigate(['GroupsMemberComponent', { groupId: id, type: 5, id: this.degreeId }]);
    }
    public selectDegreeStatus(data: any) {
        this.selectedDegreeStatus = data.value;
    }
