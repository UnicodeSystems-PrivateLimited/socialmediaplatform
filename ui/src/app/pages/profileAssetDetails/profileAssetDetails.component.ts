...
@Component({
    selector: 'data-profile-Pic',
    template: require('./profileAssetDetails.html'),
    pipes: [DateTime, LikeFilterPipe, DateFormatPipe, nameFilterPipe, YoutubeSafeUrlPipe, TimeAgoPipe, CalendarPipe, TextFormattingPipe],
    host: { 'class': 'ng-animate page1Container' },
    directives: [RouterOutlet,EmbedDirective, RouterLink,LikeMemberListComponent, BaPostExternalShareModel, ROUTER_DIRECTIVES, BaPostShareModel, Angular2AutoScroll, AddPostToJournalModal],
})

export class ProfileAssetDetailsComponent {
    public post: any;
    private _dataUrl = '/api/post/post/';
    ...
    addComment(post_id, posts) {
        ...
        if (this.postComment.comment && text.trim()) {
    ...
    public initEditor() {
        setTimeout(() => {
            let editorDiv = window["_globalJQuery"]("div#profile-inline-editor");
    ...
    public initEditEditor(message: any) {
        setTimeout(() => {
            let editorDiv = window["_globalJQuery"]("div#profile-edit-inline-editor");
            editorDiv.froalaEditor({
                toolbarInline: true,
                toolbarButtons: ['bold', 'italic', 'underline', 'strikeThrough', 'color', 'emoticons', '-', 'paragraphFormat', 'align', 'formatOL', 'formatUL', 'indent', 'outdent', '-', 'insertImage', 'insertLink', 'insertFile', 'insertVideo', 'undo', 'redo'],
            });
